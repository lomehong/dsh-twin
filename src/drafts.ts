...
import { loadCardsState, saveCards } from './cards.ts'
...
export function loadDrafts(): DraftStore {
  const p = draftsPath()
  if (!existsSync(p)) return { drafts: [] }
  try {
    const s = JSON.parse(readFileSync(p, 'utf8')) as DraftStore
...
const PATTERNS: Array<{ category: NonNullable<ExemplarDraft['category']>; test: RegExp }> = [
...
export function mineExemplars(texts: string[]): ExemplarDraft[] {
  const out: ExemplarDraft[] = []
  for (const raw of texts) {
    const text = normalizePersonaText(raw).slice(0, 600)
    if (text.length < 4) continue
    let category: ExemplarDraft['category'] = '一般'
    ...
    if (category === '一般') continue // 只挖高价值场景
    out.push({
...
/** 批量入池（同 say 指纹去重） */
export function addDrafts(drafts: ExemplarDraft[]): { added: number; duplicates: number } {
...
/** 挖掘入口：文本数组 → 规则抽取 → 入池 */
export function mineAndPool(texts: string[]): { added: number; duplicates: number; scanned: number } {
  const drafts = mineExemplars(texts)
  const r = addDrafts(drafts)
  return { ...r, scanned: texts.length }
}

export function listDrafts(state?: ExemplarDraft['state']): ExemplarDraft[] {
  const store = loadDrafts()
...
export function confirmDraft(
  draftId: string,
  opts: { regressionReportId?: string; avoidSay?: string } = {},
): { ok: boolean; error?: string } {
  const store = loadDrafts()
  const d = store.drafts.find(x => x.id === draftId)
  if (d === undefined) return { ok: false, error: '草稿不存在' }
  if (d.state !== '候选') return { ok: false, error: `草稿已处于「${d.state}」` }
  d.state = '已确认'
  d.confirmedAt = new Date().toISOString()
  if (opts.avoidSay !== undefined && opts.avoidSay !== '') d.avoidSay = opts.avoidSay.slice(0, 600)
  saveDrafts(store)
...
export function rejectDraft(draftId: string): { ok: boolean; error?: string } {
  const store = loadDrafts()
  const d = store.drafts.find(x => x.id === draftId)
  if (d === undefined) return { ok: false, error: '草稿不存在' }
  d.state = '已驳回'
