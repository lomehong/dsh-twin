...
export const BUILT_IN_FIELDS: ReadonlyArray<BuiltInFieldDef> = [
...
export const BUILT_IN_KEYS: ReadonlySet<string> = new Set(BUILT_IN_FIELDS.map(f => f.key))

/** 语气可选值（中文词，直接存入字段值） */
export const TONE_OPTIONS: ReadonlyArray<{ value: string; label: string }> = [
  { value: '专业', label: '专业' },
  { value: '亲切', label: '亲切' },
  { value: '简洁', label: '简洁' },
  { value: '幽默', label: '幽默' },
