...
const ROLE_COLOR: Record<string, string> = { master: '#1d7a53', colleague: '#3f51c1', customer: '#b06a1f', stranger: '#8a92a6', blocked: '#b03a44' }
...
const s: Record<string, React.CSSProperties> = {
  ...
  loopBtn: { float: 'right' as const, fontSize: 11, padding: '1px 8px', border: '1px solid #1d7a53', borderRadius: 4, background: '#e9f5ef', color: '#1d7a53', cursor: 'pointer' },
      ...
      {withRelations.map(p => (
        <div key={p.entity.id} style={s.card}>
          <div style={s.head}>
            ...
            <span style={{ ...s.roleBadge, background: `${ROLE_COLOR[p.entity.role] ?? '#8a92a6'}1a`, color: ROLE_COLOR[p.entity.role] ?? '#555' }}>
            ...
            <>
              ...
              {p.openLoops.map(o => (
                <div key={o.memoryId} style={s.loop}>
                  ...
                  <span style={{ color: '#b0a080', fontSize: 11, marginLeft: 8 }}>{o.openedAt.slice(0, 10)}</span>
                </div>
              ))}
            </>
          )}
          <div style={s.sec}>观察 / 推断（最近 {p.observations.length}）</div>
          {p.observations.length === 0 && <div style={{ ...s.item, color: '#b0b6c2' }}>暂无</div>}
