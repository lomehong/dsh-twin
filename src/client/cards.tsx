 ...
 * v2.2 布局（宪章审计后）：主-副两栏——左列四卡表单，右栏 sticky 常驻
 * 「保存操作 / 投影预览 / 修订史」（保存与治理状态随时可见，不再被长页
 * 滚动和 composer 遮挡分离）；wrap 底部预留 composer 高度。
 *
 * 数据链路不变：
...
const s: Record<string, React.CSSProperties> = {
  ...
  layout: { display: 'flex', gap: 16, alignItems: 'flex-start', flexWrap: 'wrap' as const },
  colMain: { flex: '1 1 640px', minWidth: 0, display: 'flex', flexDirection: 'column' as const, gap: 12 },
  colRail: { flex: '1 1 300px', maxWidth: 380, position: 'sticky' as const, top: 12, display: 'flex', flexDirection: 'column' as const, gap: 12 },
  actionRow: { display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' as const },
  ...
  fieldLabel: { fontSize: 12.5, color: 'var(--dsw-alias-label-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const },
  input: inputBase,
  textarea: { ...inputBase, minHeight: 64, resize: 'vertical' as const },
...
function CardsPage() {
  ...
  /** 未保存修改的丢弃确认（刷新 / 迁移都会覆盖本地编辑）。 */
  const confirmDiscard = (): boolean =>
    !dirty || window.confirm('当前有未保存的修改，继续将丢弃这些修改。确定继续？')
  ...
  /** 身份字段单元格：标签 + 可见性在头行，控件在下方——宽窗口下不与下拉分离。 */
  const identityCell = (def: (typeof BUILT_IN_FIELDS)[number]) => {
    ...
    const control = def.control === 'tone' ? (
      <select style={{ ...s.select, width: '100%' }} value={f?.value ?? ''} onChange={(e) => set({ value: e.target.value })}>
        <option value="">（未设置）</option>
        {TONE_OPTIONS.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
      </select>
    ) : def.control === 'textarea' ? (
      <textarea style={s.textarea} value={f?.value ?? ''} placeholder={ph}
        onChange={(e) => set({ value: e.target.value })} />
    ) : (
      <input style={s.input} value={f?.value ?? ''} placeholder={ph} onChange={(e) => set({ value: e.target.value })} />
    )
    return (
      <div key={def.key} style={s.cell}>
        <div style={s.cellHead}>
          <span style={s.fieldLabel} title={def.label}>{def.label}{def.visibility === '私密' ? ' 🔒' : ''}</span>
  ...
  const statusLine = meta.hasEffective
    ? `修订 #${meta.revisionNo} 已生效`
    : `修订 #${meta.revisionNo} 为候选——生效需要：${confirm ? '' : '主人确认 '}${regressionPassed ? '' : '回归通过'}`.trim()
      ...
      <div style={s.layout}>
        ...
        <div style={s.colMain}>
          ...
          <div style={s.card}>
            ...
            <div style={s.grid}>
              {BUILT_IN_FIELDS.map(def => identityCell(def))}
            </div>
            <div style={s.hint}>
            ...
            {cards.identity.fields.map((f, i) => (
              BUILT_IN_FIELDS.some(d => d.key === f.key) ? null : (
                <div key={`c-${i}`} style={{ ...s.itemRow, maxWidth: 640 }}>
        ...
        {/* ── 右栏（sticky）：保存操作 / 投影预览 / 修订史 ── */}
        <div style={s.colRail}>
          <div style={s.card}>
            <div style={s.cardHead}>
              <h2 style={s.cardTitle}>保存</h2>
              <span style={s.cardSub}>{statusLine}</span>
            </div>
            <div style={{ ...s.actionRow, padding: 0, border: 'none', background: 'transparent', marginBottom: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' as const, marginBottom: 8 }}>
              ...
              <button style={s.btn2} onClick={() => { if (confirmDiscard()) void save(true) }}>从旧配置迁移</button>
              <button style={s.btn2} onClick={() => { if (confirmDiscard()) void load() }}>刷新</button>
            </div>
            {msg !== null && (
              <div style={{ ...s.msg, background: msg.ok ? 'var(--dsw-alias-state-success-tertiary)' : 'var(--dsw-alias-state-warn-tertiary)', color: msg.ok ? 'var(--dsw-alias-state-success-primary)' : 'var(--dsw-alias-state-warn-label)' }}>{msg.text}</div>
            )}
          </div>
          ...
          {preview !== null && (
            <div style={s.card}>
              <div style={s.cardHead}>
                ...
                <span style={s.cardSub}>实际注入的系统提示词</span>
              </div>
