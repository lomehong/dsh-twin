/**
 * 实例级活动感知（消费端）——主人拍板：看板 = 唯一活动权威。
 *
 * 主人在**任何通道**问「在忙什么」，分身都要能感知整个实例。架构上 twin
 ...
 * - pendingApprovals：待主人审批的任务
 * - recentCompleted：最近完成的任务（含结果摘要）
 *
 * 可见性（主人拍板 3）：访客完全不可见——其他工作现场的标题可能含主人敏感事务。
 * 空闲时返回空串：零 token 成本。
 *
 ...
 * getter 返回 undefined 或 activity 缺席 → 活动区段整体降级为空（访客/主人都不注入）。
 */
 ...
 * - 主人视图：全量（任务执行现场 + 未归属自由会话 + 待审批 + 最近完成）。
 */
export function renderActivitySection(opts: { guestView: boolean }): string {
  ...
  const lines = [
    ...
    '以下现场都是你自己——同一分身在其他工作现场的实时状态。主人问「在忙什么」时，如实汇报全局活动，不要只谈当前对话。',
  ]
  if (runningTasks.length > 0) {
    lines.push(`- 进行中任务 ${runningTasks.length} 项：${runningTasks.map(t => `〈${t.title}〉${t.sessionId !== undefined ? '（执行会话运行中）' : ''}`).join('、')}`)
  }
  if (pendingApprovals.length > 0) {
    lines.push(`- 待主人审批 ${pendingApprovals.length} 项：${pendingApprovals.map(t => `〈${t.title}〉（任务号 ${t.taskId}，主人说"同意"即批准）`).join('、')}`)
  }
  if (goals.length > 0) {
