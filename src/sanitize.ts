 ...
 */
export function normalizePersonaText(input: unknown): string {
...
export function normalizePersonaLine(input: unknown): string {
  return normalizePersonaText(input).replace(/\s+/g, ' ').trim()
}

export function twinWarn(...args: unknown[]): void {
  try {
