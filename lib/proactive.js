...
export function saveProactive(store) {
    const p = proactivePath();
 ...
 */
export function ingestStateSeeds(seeds) {
    const st = loadCardsState();
    let added = 0;
    let skipped = 0;
    const state = [...st.file.current.state.items];
    const exists = (content) => state.some(s => s.content === content);
    const now = new Date();
 ...
 */
export function buildReachCandidates(deps) {
    const out = [];
    // 幂等键统一为 refKey（既有记录 note 存 refKey，见 makeRec/deliverReach）
    const seen = new Set(loadProactive().reaches.filter(r => r.status !== '降级').map(r => r.note).filter((n) => n !== undefined));
    ...
    try {
        ...
        for (const e of entries) {
            ...
            if (rel?.openLoop === undefined || rel.openLoop.closedAt !== undefined)
                continue;
            const openedAt = rel.openLoop.openedAt ?? e.timestamp ?? '';
            if (openedAt === '' || nowMs - new Date(openedAt).getTime() < dayMs)
                continue;
            ...
            if (typeof rel.actorId === 'string' && rel.actorId !== '')
                cand.actorId = rel.actorId;
            out.push(cand);
        }
    }
    catch { /* 记忆缺席 */ }
 ...
 */
export async function deliverReach(candidate, deps) {
    // 频控
    const now = Date.now();
    while (reachTimes.length > 0 && now - reachTimes[0] > REACH_WINDOW_MS)
        reachTimes.shift();
...
function makeRec(c, status, note) {
    const rec = {
        id: `PR-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
        at: new Date().toISOString(),
        kind: c.kind,
...
/** 测试钩子：清空频控窗口（进程级数组） */
export function resetReachThrottleForTest() {
    reachTimes.length = 0;
}
