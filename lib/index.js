...
export function normalizeConfigInput(body) {
  ...
  if (b.identity && typeof b.identity === 'object' && !Array.isArray(b.identity)) {
    identity.name = cleanStr(b.identity.name, 60)
    identity.role = cleanStr(b.identity.role, 80)
    identity.background = cleanStr(b.identity.background, 2000)
  }
  const persona = { ...d.persona }
  if (b.persona && typeof b.persona === 'object' && !Array.isArray(b.persona)) {
    persona.tone = TONE_VALUES.has(b.persona.tone) ? b.persona.tone : d.persona.tone
    persona.style = cleanStr(b.persona.style, 500)
    persona.values = cleanStr(b.persona.values, 1000)
    persona.rules = cleanStr(b.persona.rules, 1000)
    persona.escalation = cleanStr(b.persona.escalation, 1000)
    persona.avoid = cleanStr(b.persona.avoid, 1000)
  }
  let seeds = []
  if (Array.isArray(b.knowledge?.seeds)) {
    seeds = b.knowledge.seeds
    ...
    if (ctx.systemPrompt && typeof ctx.systemPrompt.section === 'function') {
      ...
      ctx.systemPrompt.section({
        ...
        text: (context) => {
          ...
          const actor = context?.agent?.ctx ? actorByCtx.get(context.agent.ctx) : undefined
