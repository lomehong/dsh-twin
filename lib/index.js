...
import { dshHome, legacyConfigPath, normalizePersonaText, normalizePersonaLine, twinWarn } from "./sanitize.js";
import { effectiveCards, isEffectivelyEmpty, loadCardsState, saveCards, listRevisions, migrateTwinConfigToCards } from "./cards.js";
import { renderCards, projectionSummary } from "./projection.js";
import { mineAndPool, listDrafts, confirmDraft, rejectDraft } from "./drafts.js";
import { ingestStateSeeds, pruneExpiredState, buildReachCandidates, tick, loadProactive } from "./proactive.js";
import { enqueue as learningEnqueue, confirmCandidate as learningConfirm, rejectCandidate as learningReject, applyCandidate as learningApply, listEvents as learningListEvents, listCandidates as learningListCandidates, loadEvents as learningLoadEvents, loadCandidates as learningLoadCandidates, saveEvents, saveCandidates, } from "./learning.js";
import { injectBoardGetter, renderActivitySection } from "./activity.js";
...
export { normalizePersonaText as sanitizePersonaText, normalizePersonaLine as sanitizePersonaLine } from "./sanitize.js";
 ...
 */
export function presetDefinition(deps = detectOptionalDeps()) {
    return {
        id: PRESET_ID,
        name: '数字分身',
 ...
 */
export function registerPreset(ctx) {
    try {
        ctx.inject?.(['agentPresets'], (pctx) => {
            const registry = pctx.agentPresets;
...
export function ensureDefaultPreset(ctx) {
    ctx.inject?.(['settings', 'agentPresets'], (sctx) => {
        const scope = sctx;
        const settings = scope.get('settings');
        const registry = scope.get('agentPresets');
        const write = async () => {
            const current = registry?.defaultId;
            if (current === PRESET_ID)
                return 'done';
            if (current !== undefined && current !== HOST_FALLBACK_DEFAULT)
                return 'noop';
...
// ── v2 学习闭环路由 ──
function registerLearningApi(web, ctx) {
    const disposers = [];
    ...
    disposers.push(web.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                saveCandidates(cs);
                if (c === undefined) {
                    respondJson(res, 404, { ok: false, error: '候选不存在' });
                    return;
                }
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                if (c.confirmedAt === undefined) {
                    respondJson(res, 400, { ok: false, error: '候选未确认（需要先 confirm）', candidate: c });
                    return;
                }
                let merged = false;
    ...
    disposers.push(web.register({
        ...
        handler: (_req, res) => {
            try {
                respondJson(res, 200, { ok: true, drafts: listDrafts() });
            }
            catch (e) {
                respondJson(res, 500, { ok: false, error: e instanceof Error ? e.message : String(e) });
            }
    ...
    disposers.push(web.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                const body = (await readJsonBody(req));
                const texts = Array.isArray(body.texts) ? body.texts.map(x => String(x)).slice(0, 500) : [];
...
function defaultThreshold(kind) {
    return { 纠正: 3, 否决: 2, 事实更正: 1, 影子差异: 5 }[kind];
}
// ── v3 主动触达路由 ──
function registerProactiveApi(web) {
    const disposers = [];
    ...
    disposers.push(web.register({
        ...
        handler: (_req, res) => {
            try {
                respondJson(res, 200, { ok: true, reaches: loadProactive().reaches.slice(-50) });
            }
            catch (e) {
                respondJson(res, 500, { ok: false, error: e instanceof Error ? e.message : String(e) });
            }
        ...
        ctxEvents.on?.('timer', () => {
            void (async () => {
                try {
                    // 状态卡：清理过期 + 从各源汇入
                    pruneExpiredState();
                    const memSvc = ctx.get('dsh-memory');
                    const seeds = [];
                    ...
                    try {
                        mindOwnsReach = ctx.get('dsh-mind') !== undefined;
                    }
                    catch { /* 惰性探测失败按缺席 */ }
                    if (mindOwnsReach) {
                    ...
                    const ledgerSvc = ctx.get('dsh-ledger');
                    const imSvc = ctx.get('im-channel');
                    const tickDeps = {};
                    if (ledgerSvc !== undefined)
                        tickDeps.ledger = ledgerSvc;
                    if (memSvc !== undefined)
                        tickDeps.memory = memSvc;
                    if (imSvc !== undefined)
                        tickDeps.im = imSvc;
                    await tick(tickDeps);
                }
